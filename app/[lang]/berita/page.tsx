  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
      <main className="max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8 border-b-2 border-[#0b5932] inline-block pb-2">
          <h1 className="text-3xl font-black text-gray-900 uppercase tracking-tight">{ui.indexTitle}</h1>
        </div>

        <div className="lg:grid lg:grid-cols-12 lg:gap-10">
          <div className="lg:col-span-9">
    
            <Link href={`/${lang}/berita/${headline.slug}`} className="group block bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-10 hover:shadow-lg transition duration-300">
              <div className="relative h-[400px] overflow-hidden">
                ...
                <div className="absolute top-4 left-4 bg-red-600 text-white text-xs font-black px-3 py-1 rounded uppercase tracking-widest animate-pulse">{ui.highlight}</div>
              </div>
              <div className="p-6 md:p-8">
                <span className="text-xs font-bold text-[#0b5932] bg-[#0b5932]/10 px-3 py-1.5 rounded mb-4 inline-block">{headline.tags[0]}</span>
                <h2 className="text-3xl md:text-4xl font-black text-gray-900 group-hover:text-[#0b5932] leading-tight mb-4 transition">{headline.title}</h2>
                <div 
            ...
            <h3 className="text-xl font-black text-gray-900 mb-6 flex items-center gap-2 uppercase tracking-wide border-b-2 border-gray-200 pb-2">{ui.latest}</h3>
              ...
              {newsList.map(item => (
                <Link href={`/${lang}/berita/${item.slug}`} key={item.slug} className="group bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition duration-300 flex flex-col">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <h4 className="text-base font-bold text-gray-800 group-hover:text-[#0b5932] leading-snug mb-3 line-clamp-3 flex-1">{item.title}</h4>
            ...
            <div className="bg-[#0b5932] p-8 rounded-2xl text-center text-white shadow-lg mb-10 lg:mb-0">
              <h4 className="text-2xl font-black mb-2 uppercase tracking-wide">{ui.subscribe}</h4>
              <div className="flex max-w-md mx-auto gap-2 mt-6">
                <input type="email" placeholder={ui.emailPlaceholder} className="flex-1 px-4 py-3 rounded-lg text-gray-900 bg-white placeholder-gray-500 text-sm focus:outline-none shadow-inner" />
                <button className="bg-emerald-600 text-white border border-emerald-400 font-bold px-6 py-3 rounded-lg hover:bg-emerald-500 transition text-sm">{ui.btnSubscribe}</button>
              </div>
            </div>
          </div>
